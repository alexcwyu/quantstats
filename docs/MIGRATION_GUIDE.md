...
- `.flake8` (replaced by `[tool.ruff]` in pyproject.toml)
- `.deepsource.toml` (no longer used)
- `.travis.yml` (legacy CI, replaced by GitHub Actions)
- `QuantStats.egg-info/` (build artifact)
- `dist/` (build artifact)
...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `quantstats/` -> `src/quantstats/`
- **Import unchanged**: `import quantstats` still works

If upstream adds files to the old location, move them to `src/quantstats/`.
...
1. **Delete re-added legacy files**: `.flake8`, `.deepsource.toml`, `.travis.yml`
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/quantstats/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import quantstats" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import quantstats"       # Verify import
